...
module Quarks
  class Installer
    ...
    def install_unlocked(dest_dir)
      ...
      validate_registered_paths!(backup_paths, install_root)
    ...
    def uninstall_unlocked
      ...
      validate_registered_paths!(files, install_root)
    ...
    def validate_registered_paths!(paths, install_root)
      root = File.expand_path(install_root)
      raise InstallError, "Install root must not be a symlink: #{root}" if File.symlink?(root)

      Array(paths).each do |rel|
        relative = rel.to_s.sub(%r{\A/+}, "")
        raise InstallError, "Invalid registered package path" if relative.empty? || relative.include?("\0")
        target = File.expand_path(relative, root)
        unless Security.path_within?(target, root, allow_root: false)
          raise InstallError, "Registered package path escapes the install root: #{rel}"
        end
        validate_target_parent!(target, root)
      end
      true
